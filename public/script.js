        // Global state
        const API_URL ='https://shedlike-cyndi-unapprovingly.ngrok-free.dev/api/auth';
        ...
        async function initializeRakshaAI() {
            try {
                // Check if we're in native app (Capacitor)
                if (window.Capacitor) {
                    ...
                    if (RakshaAI) {
                        ...
                        console.log('✅ Raksha AI initialized successfully');
                    }
                } else {
                    console.log('📱 Running in web mode - AI features limited');
                }
            } catch (error) {
        ...
        async function askAIMedicalQuestion(question, language = 'EN') {
            if (!isAIInitialized || !rakshaAI) {
                return getDemoAIResponse(question, language);
            }

            try {
                // Create context-aware prompt based on user's language
                let prompt = '';
                if (language === 'HI') {
                    prompt = `आप एक चिकित्सा सहायक हैं। सरल हिंदी में उत्तर दें: ${question}`;
                } else if (language === 'KN') {
                    prompt = `ನೀವು ವೈದ್ಯಕೀಯ ಸಹಾಯಕರು. ಸರಳ ಕನ್ನಡದಲ್ಲಿ ಉತ್ತರಿಸಿ: ${question}`;
                } else {
                    prompt = `You are a medical assistant for elderly patients. Answer in simple terms: ${question}`;
                }

                const result = await rakshaAI.askMedicalQuestion({ question: prompt });
        ...
        // Demo responses for web mode
        function getDemoAIResponse(question, language) {
            const responses = {
                'EN': `I understand you're asking about: "${question}". In the mobile app, I'll provide personalized medical guidance based on your health history. For now, please consult your doctor for specific medical advice.`,
                'HI': `मैं समझता हूं आप पूछ रहे हैं: "${question}"। मोबाइल ऐप में, मैं आपके स्वास्थ्य इतिहास के आधार पर व्यक्तिगत चिकित्सा मार्गदर्शन प्रदान करूंगा। अभी के लिए, विशिष्ट चिकित्सा सलाह के लिए अपने डॉक्टर से परामर्श करें।`,
                'KN': `ನೀವು ಕೇಳುತ್ತಿರುವುದನ್ನು ನಾನು ಅರ್ಥಮಾಡಿಕೊಂಡಿದ್ದೇನೆ: "${question}". ಮೊಬೈಲ್ ಆ್ಯಪ್‌ನಲ್ಲಿ, ನಾನು ನಿಮ್ಮ ಆರೋಗ್ಯ ಇತಿಹಾಸದ ಆಧಾರದ ಮೇಲೆ ವೈಯಕ್ತಿಕ ವೈದ್ಯಕೀಯ ಮಾರ್ಗದರ್ಶನವನ್ನು ಒದಗಿಸುತ್ತೇನೆ. ಈಗ, ನಿರ್ದಿಷ್ಟ ವೈದ್ಯಕೀಯ ಸಲಹೆಗಾಗಿ ದಯವಿಟ್ಟು ನಿಮ್ಮ ವೈದ್ಯರನ್ನು ಸಂಪರ್ಕಿಸಿ.`
            };
        ...
        // AI Health Record Analyzer
        async function analyzeHealthRecordWithAI(recordData) {
            const analysisModal = `
                <div id="aiAnalysisModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div class="bg-white rounded-3xl p-6 w-full max-w-md">
                        <div class="text-4xl mb-4 text-center">📊</div>
                        <h3 class="text-xl font-bold mb-4 text-center">AI Health Analysis</h3>
                        <div class="bg-blue-50 rounded-xl p-4 mb-4">
                            <div class="flex items-center justify-center">
                                <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                                <span class="ml-3 text-blue-800">Analyzing your health data...</span>
                            </div>
                        </div>
            ...
            try {
                ...
                const prompt = `Analyze this health data for a ${user.age} year old ${user.gender}:
        ...
        Additional Data: ${recordData}
            
        Provide a brief, helpful analysis with recommendations in simple language.`;

                const analysis = await askAIMedicalQuestion(prompt, currentLanguage);

                // Show result
                document.querySelector('#aiAnalysisModal .bg-blue-50').classList.add('hidden');
                document.getElementById('analysisResult').classList.remove('hidden');
                document.getElementById('analysisResult').innerHTML = `
                    <div class="bg-green-50 rounded-xl p-4 mb-4">
                        <div class="font-medium text-green-900 mb-2">📋 AI Analysis:</div>
                        <div class="text-sm text-green-800 whitespace-pre-wrap">${analysis}</div>
                    </div>
                    <button onclick="closeAIAnalysisModal()" class="w-full bg-blue-500 text-white py-3 rounded-xl font-semibold">
                        Close
                    </button>
                `;
        ...
        // Add AI analysis button to health status
        function addAIAnalysisButton() {
            return `
                <button onclick="analyzeMyHealth()" class="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white py-4 rounded-2xl font-semibold mb-4 shadow-lg">
        ...
        Health Conditions: ${(user.healthConditions || []).join(', ') || 'None'}
            ...
            await analyzeHealthRecordWithAI(healthSummary);
        }
        ...
        // AI Medical Assistant Screen
        function showAIAssistant() {
            const content = `
                <div class="space-y-4">
                    <div class="bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl p-6 text-white text-center">
                        <div class="text-6xl mb-4">🤖</div>
                        <h2 class="text-2xl font-bold mb-2">AI Medical Assistant</h2>
                        <p class="text-sm opacity-90">Ask me anything about health, medicines, or symptoms</p>
                    </div>
                    
                    <div class="bg-white rounded-2xl p-6 card-shadow">
                        <h3 class="font-bold text-lg mb-4">💬 Ask Your Question</h3>
                        <textarea id="aiQuestionInput" rows="4" class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 mb-4" placeholder="Example: What should I do if I have fever and headache?"></textarea>
                        <button onclick="askAIQuestion()" class="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white py-4 rounded-2xl font-semibold">
                            Ask AI Assistant
                        </button>
                    </div>
                    
                    <div id="aiResponseContainer" class="hidden bg-white rounded-2xl p-6 card-shadow">
                        <h3 class="font-bold text-lg mb-4 flex items-center">
                            <span class="text-2xl mr-2">🤖</span>
                            AI Response
                        </h3>
                        <div id="aiResponseText" class="bg-blue-50 rounded-xl p-4 text-gray-800"></div>
                    </div>
                    
                    <div class="bg-white rounded-2xl p-6 card-shadow">
                        <h3 class="font-bold text-lg mb-4">💡 Quick Questions</h3>
                        <div class="space-y-2">
                            <button onclick="askQuickQuestion('What are the symptoms of diabetes?')" class="w-full text-left p-3 bg-purple-50 hover:bg-purple-100 rounded-xl text-sm">
                                What are the symptoms of diabetes?
                            </button>
                            <button onclick="askQuickQuestion('How can I lower my blood pressure naturally?')" class="w-full text-left p-3 bg-purple-50 hover:bg-purple-100 rounded-xl text-sm">
                                How can I lower my blood pressure naturally?
                            </button>
                            <button onclick="askQuickQuestion('What foods are good for heart health?')" class="w-full text-left p-3 bg-purple-50 hover:bg-purple-100 rounded-xl text-sm">
                                What foods are good for heart health?
                            </button>
                            <button onclick="askQuickQuestion('When should I see a doctor for fever?')" class="w-full text-left p-3 bg-purple-50 hover:bg-purple-100 rounded-xl text-sm">
            ...
            `;
            showFeature('AI Medical Assistant', content);
        }
        ...
        async function askAIQuestion() {
            ...
            responseText.innerHTML = '<div class="flex items-center justify-center py-4"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600"></div><span class="ml-3 text-purple-800">AI is thinking...</span></div>';
            
            try {
                // Get AI response
                const response = await askAIMedicalQuestion(question, currentLanguage);
                
                // Show response
                responseText.innerHTML = `<div class="whitespace-pre-wrap">${response}</div>`;
                ...
                console.error('AI Question Error:', error);
                responseText.innerHTML = '<div class="text-red-600">Sorry, I encountered an error. Please try again.</div>';
            }
        }
        ...
        // Enhanced AI Voice Assistant - Medical Q&A
        async function handleVoiceQuery(query) {
            speak('Let me think about that...');

            try {
                const response = await askAIMedicalQuestion(query, currentLanguage);
                speak(response);

                // Show response visually too
                const responseModal = `
                    <div id="aiResponseModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                        <div class="bg-white rounded-3xl p-6 w-full max-w-md">
                            <div class="text-4xl mb-4 text-center">🤖</div>
                            <h3 class="text-xl font-bold mb-4 text-center">AI Medical Assistant</h3>
                            <div class="bg-blue-50 rounded-xl p-4 mb-4">
                                <div class="font-medium text-blue-900 mb-2">Your Question:</div>
                                <div class="text-sm text-blue-800">${query}</div>
                            </div>
                            <div class="bg-green-50 rounded-xl p-4 mb-6">
                                <div class="font-medium text-green-900 mb-2">AI Response:</div>
                                <div class="text-sm text-green-800">${response}</div>
                            </div>
                            <button onclick="closeAIResponseModal()" class="w-full bg-blue-500 text-white py-3 rounded-xl font-semibold">
                                Got it, Thanks!
                            </button>
                        </div>
                    </div>
        ...
        // Add medical question commands to voice assistant
        function initializeAIVoiceCommands() {
            if (annyang) {
                const medicalCommands = {
                    'what is *question': (question) => { handleVoiceQuery('what is ' + question); },
                    'how do I *question': (question) => { handleVoiceQuery('how do I ' + question); },
                    'can I *question': (question) => { handleVoiceQuery('can I ' + question); },
                    'should I *question': (question) => { handleVoiceQuery('should I ' + question); }
                };

                annyang.addCommands(medicalCommands);
            }
        }
